function [level em] = graythresh_origin(I)
...
if ~isempty(I)
    ...
    I = im2uint8(I(:));
    num_bins = 256;
    counts = imhist(I,num_bins); % 每一级灰度级像素数集合
    
    % Variables names are chosen to be similar to the formulas in
    % the Otsu paper.
    p = counts / sum(counts);
    omega = cumsum(p); % 每一级灰度级的概率
    mu = cumsum(p .* (1:num_bins)'); % 总体期望
    mu_t = mu(end); %
    
    sigma_b_squared = (mu_t * omega - mu).^2 ./ (omega .* (1 - omega)); %判别度量
